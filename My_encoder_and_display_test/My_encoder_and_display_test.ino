...
const unsigned char DryTime[4] = {40, 20, 30, 40};
      ...
      case Time4TempMeasure: {
        ...
        if (  ( Current_tempr_value > TemperatureMaxWorkRange ) || (Current_tempr_value  < TemperatureMinWorkRange)  ) {
          ...
          // todo Go to sensor Error Mode
            ...
            regulator.input = Current_tempr_value;   // сообщаем регулятору текущую температуру
            // getResult возвращает значение для управляющего устройства
            if (HIGH == regulator.getResult())
              {
              digitalWrite(PowerRele_pin, HIGH);  // отправляем на реле
              lcd.setCursor(0, 0);  
              lcd.print("C");               
              }
            else
              {
              digitalWrite(PowerRele_pin, LOW);  // отправляем на реле 
              lcd.setCursor(0, 0);  
              lcd.print("H");               
              }
